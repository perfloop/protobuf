...
[[noreturn]] PROTOBUF_PRESERVE_ALL PROTOBUF_EXPORT void BasePointerFailure(
...
class BasePointer {
 ...
 public:
  ...
  BasePointer(T* ptr, const void* base) {
    ...
    if (ABSL_PREDICT_FALSE((!kAllowNull && ptr == nullptr) ||
                           static_cast<int32_t>(diff) != diff)) {
      BasePointerFailure(ptr, base);
    }

    if constexpr (kAllowNull) {
      if (ABSL_PREDICT_FALSE(diff == kNullOffset)) {
        BasePointerFailure(ptr, base);
      }
    }
